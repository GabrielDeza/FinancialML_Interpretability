...
from v2_fixed_helper_financial_metrics import *
...
file_body = f"_prediction_length={prediction_length}_BS={batch_size}_NBpE={num_batches_per_epoch}_epochs={epochs}" \
               f"_parameter={parameter}_frac_amount={frac_amount}_dir={direction}_eps={epsilon}_adv_example_type={adv_example_type}"
...
perturbations = {'Type of Algorithm': f"{adv_example_type}_{direction}_while_{parameter}_regular_grad", 'perturbation size': epsilon,
        'features': column_counter, 'time_features':{}}
...
if direction ==1:
    while (np.mean(next_distr_params) < frac*np.mean(original_distr_params)) and iter_num <= 100:
                    ...
                    for t in range(0,prediction_length):
                        ...
                        for tries in range(0,len(columns)):
                            ...
                            if after < max_values[columns[idx]] and after > min_values[columns[idx]]:
                                ...
                                feats_perturbed.append(columns[idx])
                                #The actual change occured, lets record it
                                break
    perturbations['time_features'][iter_num]=feats_perturbed
if direction == -1:
    while (np.mean(next_distr_params) > frac*np.mean(original_distr_params)) and iter_num <= 100:
        # number of times to iterate over testing set
        print(f"On iter {iter} original val = {np.mean(original_distr_params)} need to beat ={frac*np.mean(original_distr_params)} curr val = {np.mean(next_distr_params)} ")
                ...
                for t in range(0, prediction_length):
                    ...
                    for tries in range(0, len(columns)):
                        ...
                        if after < max_values[columns[idx]] and after > min_values[columns[idx]]:
                            ...
                            break
    perturbations['time_features'][iter_num] = feats_perturbed
perturbations['num iters'] = iter_num
