...
parser.add_argument('--max_iter', default=4, type=str, help='number of iterations on the adv dataset algorithm')
...
my_list = [str(item) for item in args.max_iter.split(',')]
max_iter = my_list[0].split()
max_iter = [int(i) for i in max_iter]

my_list = [str(item) for item in args.parameter.split(',')]
parameter = my_list[0].split()

big_list = [companies, aet,train_length,validation_length, test_length, prediction_length, epochs, batch_size, nbpe, seed, adv_dir, epsilon, max_iter, parameter]
...
for instance in combinations:
    ...
    with open(job_file, 'w+') as fh:
        ...
        fh.writelines(f"python -u Updated_{model}-Log_diff-{freq}_Attacked.py --company {c} --bit {args.bit} --adv_example_type {aaa} --train_length {tl} --validation_length {vl} --test_length {test_l} --prediction_length {pl} --epochs {e} --batch_size {b} --nbpe {num_b_p_e} --seed {s} --adv_dir {d} --epsilon {eps} --max_iter {max_iter_single} --parameter {param}\n")
