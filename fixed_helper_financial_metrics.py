...
def kelly_greedy_gain(period_dates, pred, target,df_open,df_close,freq_type ='day'):
    f_list = []
    if freq_type =='day':
        freq_str = '%Y-%m-%d'
    if freq_type =='hour':
    ...
    for i in range(period_dates.shape[0]):
        ...
        if pred[i] > 0:
            if len(returns)==0:
                ...
                W = len(ret[ret > 0]) / len(ret)
                R = ret[ret > 0].mean() / np.abs(ret[ret < 0].mean())
                f = W - ((1 - W) / R)
            if f >= 1.0:
                f = 1.0
            if f <= 0.0:
...
def kelly_threshold_gain(period_dates, pred, target,df_open,df_close,freq_type ='day'):
    if freq_type =='day':
        freq_str = '%Y-%m-%d'
    if freq_type =='hour':
        freq_str = '%Y-%m-%d %H:%M:%S'
