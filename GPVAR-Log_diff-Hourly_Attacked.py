...
file_head = f"{company}_GPVAR_Hourly_"
...
filename = f"./Hourly_data/4-hourly-final_data/{company}_1h.csv"
...
grad_str = f"./gradient_dir/gpvar_{company}" +grad_str
        ...
        for dd, (test_entry, forecast) in enumerate(zip(interim_adv_ds, predictor.predict(interim_adv_ds))):
            for t in range(0,prediction_length):
                ...
                for tries in range(0,len(columns)-3):
                    ...
                    if adv_example_type =='simple':
                        after = adv_target[idx,time_idx] * (1+ direction * epsilon)
